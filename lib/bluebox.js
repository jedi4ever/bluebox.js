...
Bluebox.prototype._request = function(method, url, parameters, callback) {
  ...
  var methodParams = hashmerge(parameters, {});
  var getURL = Bluebox.API_URL + url.replace(/^\/+/, '');

  // Convert method name: 'del' -> 'delete' for axios
  var axiosMethod = method === 'del' ? 'delete' : method;

  // Convert parameters to URL-encoded form data
  var formData = new URLSearchParams();
  Object.keys(methodParams).forEach(function(key) {
  ...
  var axiosConfig = {
    ...
    url: getURL,
    auth: {
      username: this.credentials().customer_id,
    ...
    // Disable proxy for direct connections (needed for testing with nock)
    proxy: false
  };

  // Add form data for non-GET requests, query params for GET
  if (axiosMethod === 'get') {
    axiosConfig.params = methodParams;
  } else {
